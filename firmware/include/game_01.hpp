 ...
 * @description: This file implements the logic for Game 01 (Wires). 
 * @date       : 27-03-2026
 * @version    : 1.1
...
public:
  Game01(Timer* timer, Buzzer* buzzer, Wires* wires) : 
    _timer(timer), _buzzer(buzzer), _wires(wires), _active(false), _win(false), _lose(false) {
  }
  ...
  uint8_t loop(uint64_t millis) {
    ...
    if (this->_wires->isWin()) {
      this->_win    = true;
      this->_active = false;
    }

    if (this->_timer->isTimerZero() || this->_wires->isLose()) {
      this->_lose   = true;
      this->_active = false;
    }
